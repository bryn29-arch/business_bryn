...
def limpiar_monto_entero(val):
    """Convierte celdas de moneda/texto a número entero para evitar errores de coma/punto."""
    if pd.isna(val):
        return 0
...
# -----------------------------------------------------------------------------
# ALGORITMO SUBSET SUM (BÚSQUEDA DE COMBINACIONES EXACTAS)
# -----------------------------------------------------------------------------
def buscar_combinacion_exacta(df_candidatos, monto_objetivo, col_monto='Monto_Real', max_docs=30):
    """
    ...
    # Filtrar facturas que superen individualmente el monto del abono
    ...
    def backtrack(idx, acumulado, combo):
        ...
        if acumulado > monto_objetivo or idx >= len(items) or len(combo) >= max_docs:
            return

        # Poda si la suma restante no alcanza el objetivo
        restante = sum(m for _, m in items[idx:])
        if (acumulado + restante) < monto_objetivo:
            return
...
def conciliar_informacion(df_cartola, df_ventas):
    ...
    for idx_c, row_c in df_cartola.iterrows():
        ...
        # ---------------------------------------------------------------------
        # CAPA 1: BUSQUEDA STRICTA POR RUT
        # ---------------------------------------------------------------------
        ...
        if not match_indices and monto_banco > 0:
            ...
            for idx_v, row_v in ventas_disponibles.iterrows():
                similitud = SequenceMatcher(None, texto_c_norm, row_v['Fila_Texto_Norm']).ratio()
                if similitud >= 0.40:
