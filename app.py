...
# -----------------------------------------------------------------------------
# DICCIONARIOS Y FUNCIONES DE NORMALIZACIÓN
# -----------------------------------------------------------------------------
ABREVIATURAS = {
    "CORP": "CORPORACION",
    "EDUC": "EDUCACION",
...
def expandir_y_limpiar_texto(texto: str) -> str:
...
def limpiar_monto_entero(val) -> int:
    """Convierte celdas de moneda/texto a número entero para evitar errores de formato."""
    if pd.isna(val):
        return 0
...
# -----------------------------------------------------------------------------
# ALGORITMO SUBSET SUM OPTIMIZADO (O(1) PRUNING CON SUFFIX SUMS)
# -----------------------------------------------------------------------------
def buscar_combinacion_exacta(
    df_candidatos, monto_objetivo, col_monto="Monto_Real", max_docs=25
):
    """Encuentra un subconjunto de facturas cuyas sumas igualen exactamente el monto objetivo.

    Aplica podas con suma de sufijos en O(1).
    """
    if df_candidatos.empty or monto_objetivo <= 0:
        return []

    # Filtrar valores mayores al objetivo
    ...
    # Precalculo de sumas sufijas para poda instantánea O(1)
    suffix_sums = [0] * (n + 1)
    ...
    def backtrack(idx, acumulado, combo):
        ...
        # Poda O(1) si el acumulado más el resto no alcanza el objetivo
...
def conciliar_informacion(df_cartola, df_ventas):
    ...
    col_monto_v = next(
        (
            ...
            if c in df_ventas_prep.columns
        ),
        None,
    )
    col_folio_v = next(
    ...
    col_cliente_v = next(
        ...
        df_ventas_prep.columns[0],
    )
    col_monto_c = next(
        (
            c
            for c in ["Abono", "Monto", "Crédito", "Credito", "Deposito", "DEPOSITO"]
            if c in df_cartola.columns
        ),
        None,
    )

    # Preprocesamiento vectorizado inicial
    df_ventas_prep["Fila_Texto"] = df_ventas_prep.apply(
        lambda r: " ".join([str(v) for v in r.values if pd.notna(v)]), axis=1
    )
    df_ventas_prep["Fila_Texto_Norm"] = df_ventas_prep["Fila_Texto"].apply(
        expandir_y_limpiar_texto
    )
    df_ventas_prep["RUT_Norm"] = df_ventas_prep["Fila_Texto"].apply(
        extraer_rut
    )

    if col_monto_v:
        df_ventas_prep["Monto_Real"] = df_ventas_prep[col_monto_v].apply(
            limpiar_monto_entero
        )
    else:
        df_ventas_prep["Monto_Real"] = df_ventas_prep.apply(
            lambda r: max([limpiar_monto_entero(v) for v in r.values] + [0]),
            axis=1,
        )
    ...
    for idx_c, row_c in df_cartola.iterrows():
        ...
        monto_banco = (
            limpiar_monto_entero(row_c[col_monto_c])
            if col_monto_c
            else max([limpiar_monto_entero(v) for v in row_c.values] + [0])
        )

        match_indices = []
        tipo_match = "Sin Coincidencia"

        ventas_disponibles = df_ventas_prep[
            ~df_ventas_prep.index.isin(indices_ventas_usados)
        ]
        ...
        if rut_c and monto_banco > 0:
            ...
            if not cand_rut.empty:
                ...
                elif cand_rut["Monto_Real"].sum() == monto_banco:
                    match_indices = cand_rut.index.tolist()
                    tipo_match = f"RUT Todas las Facturas (1:{len(match_indices)})"
                else:
                    combo = buscar_combinacion_exacta(
                        cand_rut, monto_banco, col_monto="Monto_Real"
                    )
                    if combo:
                        match_indices = combo
        ...
        # ---------------------------------------------------------------------
        # CAPA 2: BÚSQUEDA AGRUPADA POR NOMBRE / TEXTO
        # ---------------------------------------------------------------------
        if not match_indices and monto_banco > 0:
            indices_cand_nombre = []
            tokens_c = set(texto_c_norm.split())

            for idx_v, row_v in ventas_disponibles.iterrows():
                tokens_v = set(row_v["Fila_Texto_Norm"].split())
                # Pre-filtro rápido por solapamiento de palabras para acelerar SequenceMatcher
                if tokens_c & tokens_v:
                    similitud = SequenceMatcher(
                        None, texto_c_norm, row_v["Fila_Texto_Norm"]
                    ).ratio()
                    if similitud >= 0.40:
            ...
            if indices_cand_nombre:
                ...
                if not exacto_nom.empty:
                    ...
                    combo_nom = buscar_combinacion_exacta(
                        cand_nom, monto_banco, col_monto="Monto_Real"
                    )
                    if combo_nom:
        ...
        # ---------------------------------------------------------------------
        # CAPA 3: MONTO ÚNICO GLOBAL (SIN AMBIGÜEDAD)
        # ---------------------------------------------------------------------
        if not match_indices and monto_banco > 0:
            cand_monto_unico = ventas_disponibles[
                ventas_disponibles["Monto_Real"] == monto_banco
            ]
            if len(cand_monto_unico) == 1:
                match_indices = [cand_monto_unico.index[0]]
                tipo_match = "Monto Único Coincidente"
        ...
        if match_indices:
            ...
            folios = ", ".join(
                [
                    str(r[col_folio_v]) if col_folio_v else f"FILA-{i+1}"
                    for i, r in rows_matched.iterrows()
                ]
            )
    ...
    ventas_pendientes = df_ventas_prep[
        ~df_ventas_prep.index.isin(indices_ventas_usados)
    ].copy()
...
st.markdown(
    "Cruce estructurado por **Agrupación de RUT** y **Algoritmo de Sumas Exactas (1 a N)**."
)
...
with col1:
    ...
    file_cartola = st.file_uploader(
        "Subir Cartola (Excel / CSV)", type=["xlsx", "xls", "csv"], key="cartola"
    )

with col2:
    st.subheader("2. Registro de Ventas / Cartera")
    file_ventas = st.file_uploader(
        "Subir Cartera / Ventas (Excel / CSV)", type=["xlsx", "xls", "csv"], key="ventas"
    )

if file_cartola and file_ventas:
    try:
        df_cartola = (
            pd.read_csv(file_cartola)
            if file_cartola.name.endswith(".csv")
            else pd.read_excel(file_cartola)
        )
        df_ventas = (
            pd.read_csv(file_ventas)
            if file_ventas.name.endswith(".csv")
            else pd.read_excel(file_ventas)
        )
        ...
        if st.button("🚀 Ejecutar Conciliación Inteligente", type="primary"):
            with st.spinner("Procesando cruce agrupado..."):
                df_cruce, df_pendientes = conciliar_informacion(
                    df_cartola, df_ventas
                )
                # Persistencia en session_state para evitar reseteos al descargar
                st.session_state["df_cruce"] = df_cruce
                st.session_state["df_pendientes"] = df_pendientes

        # Renderizado desde session_state
        if "df_cruce" in st.session_state:
            ...
            tot_exactos = len(
                df_cruce[
                    df_cruce["Estado Conciliación"] == "🟢 Conciliado Exacto"
                ]
            )
            tot_diferencias = len(
                df_cruce[
                    df_cruce["Estado Conciliación"] == "🟡 Diferencia en Monto"
                ]
            )
            tot_no_encontrados = len(
                df_cruce[
                    df_cruce["Estado Conciliación"]
                    == "🔴 Abono No Identificado"
                ]
            )
            monto_no_ident = df_cruce[
                df_cruce["Estado Conciliación"] == "🔴 Abono No Identificado"
            ]["Monto Banco ($)"].sum()
            ...
            with pd.ExcelWriter(output, engine="openpyxl") as writer:
                df_cruce.to_excel(
                    writer, sheet_name="Cartola_Conciliada", index=False
                )
                if not df_pendientes.empty:
                    df_pendientes.to_excel(
                        writer, sheet_name="Facturas_Pendientes", index=False
                    )
