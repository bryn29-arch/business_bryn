import streamlit as stimport pandas as pdimport numpy as npimport unicodedataimport refrom itertools import combinationsfrom difflib import SequenceMatcherimport io
...
st.set_page_config(page_title="Conciliación Bancaria Avanzada - Sumas Asertivas",page_icon="📊",layout="wide")
...
ABREVIATURAS = {'CORP': 'CORPORACION','EDUC': 'EDUCACION','LIMITADA': 'LTDA','SOCIEDAD': 'SOC','ANONIMA': 'SA','HERMANOS': 'HROS','EIRL': '','SPA': '','S': 'SPA','EXP': 'EXPORTACION','ING': 'INGENIERIA','SERV': 'SERVICIOS'}

def extraer_rut(texto):"""Extrae RUT sin puntos ni guion para cruces exactos (ej: 76334370K)."""if not isinstance(texto, str) or pd.isna(texto):return ""match = re.search(r'\b(\d{1,2}.?\d{3}.?\d{3}-?[\dkK])\b', str(texto))if match:return re.sub(r'[^0-9K]', '', match.group(1).upper())return ""

def expandir_y_limpiar_texto(texto):"""Normaliza texto, remueve acentos y estandariza abreviaturas."""if not isinstance(texto, str) or pd.isna(texto):return ""
...
def extraer_tokens_clave(texto_fila):"""Extrae tokens significativos de 2 o más caracteres."""texto_clean = expandir_y_limpiar_texto(texto_fila)palabras = set(re.findall(r'\b[A-Z0-9]{2,}\b', texto_clean))stopwords = {'TRASPASO', 'TRANSFERENCIA', 'BANCO', 'PAGO', 'DEBITO', 'CREDITO','INTERNET', 'VALE', 'VISTA', 'CHILE', 'CANAL', 'INFORMACION', 'TEF','ABONO', 'CARGO', 'LTDA', 'SOC', 'SA', 'SPA'}return palabras - stopwords

def calcular_similitud_textual(texto1, texto2):"""Evalúa coincidencia combinando contención directa, Jaccard por tokens y Fuzzy Ratio."""if not texto1 or not texto2:return 0.0
...
@st.cache_datadef conciliar_informacion_flexible(df_cartola, df_ventas):if df_cartola.empty or df_ventas is None or df_ventas.empty:return pd.DataFrame(), pd.DataFrame()
...
st.title("🏦 Sistema de Conciliación Bancaria Inteligente")st.markdown("Cruce avanzado con coincidencia textual reforzada y agrupación de facturas (1 a N).")

col1, col2 = st.columns(2)

with col1:st.subheader("1. Cartola Bancaria")file_cartola = st.file_uploader("Subir Cartola (Excel / CSV)", type=["xlsx", "xls", "csv"], key="cartola")

with col2:st.subheader("2. Registro de Ventas / Cartera")file_ventas = st.file_uploader("Subir Cartera / Ventas (Excel / CSV)", type=["xlsx", "xls", "csv"], key="ventas")

if file_cartola and file_ventas:try:df_cartola = pd.read_csv(file_cartola) if file_cartola.name.endswith('.csv') else pd.read_excel(file_cartola)df_ventas = pd.read_csv(file_ventas) if file_ventas.name.endswith('.csv') else pd.read_excel(file_ventas)
...
else:st.info("👆 Por favor sube ambos archivos para iniciar el cruce de información.")
