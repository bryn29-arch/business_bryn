...
def super_limpiar(texto):
    ...
    Ejemplo: 
        'Pro Drilling S A'   -> 'PRODRILLINGSA'
        'PRODRILLING S.A'    -> 'PRODRILLINGSA'
        'Proveedores En Linea' -> 'PROVEEDORESENLINEA'
    """
    if not isinstance(texto, str) or pd.isna(texto):
        return ""
    # Quitar acentos / diacríticos
    texto = unicodedata.normalize('NFD', texto).encode('ascii', 'ignore').decode("utf-8")
    # Convertir a mayúsculas y dejar solo letras y números (elimina espacios, puntos, guiones, etc.)
    texto = re.sub(r'[^A-Z0-9]', '', texto.upper())
    return texto

def calcular_similitud(s1, s2):
    """Retorna un porcentaje de similitud entre dos cadenas superlimpias (0.0 a 1.0)."""
    if not s1 or not s2:
        return 0.0
...
def conciliar_informacion_flexible(df_cartola, df_ventas):
    ...
    # Pre-calcular versiones superlimpias para acelerar las búsquedas
    ...
    df_ventas_prep['Nombre_Super_Limpio'] = df_ventas_prep['Nombre Cliente'].apply(super_limpiar)

    for idx_c, row_c in df_cartola.iterrows():
        glosa_raw = str(row_c.get('Descripción Glosa', ''))
        identificador_raw = str(row_c.get('Identificador / Cliente', glosa_raw))
        monto_pago = float(row_c.get('Monto Pago', 0.0))

        # Cadenas superlimpias de la cartola
        glosa_clean = super_limpiar(glosa_raw)
        identificador_clean = super_limpiar(identificador_raw)

        # Filtrar facturas que aún no han sido pareadas
        ventas_disponibles = df_ventas_prep[~df_ventas_prep['Folio'].isin(facturas_usadas)].copy()

        match_encontrado = None
        tipo_match = "Sin Coincidencia"

        # -------------------------------------------------------------
        # ESTRATEGIA 1: Cliente/Glosa Coincide (Compresión) + Monto Exacto
        # -------------------------------------------------------------
        for idx_v, row_v in ventas_disponibles.iterrows():
            ...
            monto_v = float(row_v['Monto Total'])

            # Validación de coincidencia por inclusión o similitud
            coincide_nombre = (
                ...
                calcular_similitud(identificador_clean, nombre_v_clean) > 0.75
            )
        ...
        # -------------------------------------------------------------
        # ESTRATEGIA 2: Coincidencia por Monto Exacto (Ranking por Nombre)
        # -------------------------------------------------------------
        if match_encontrado is None:
            match_monto = ventas_disponibles[abs(ventas_disponibles['Monto Total'] - monto_pago) < 1.0]
        ...
        if match_encontrado is None:
            ...
            if candidatos_nombre:
                ...
                df_cand['dif_abs'] = (df_cand['Monto Total'] - monto_pago).abs()
                match_encontrado = df_cand.sort_values(by='dif_abs').iloc[0]
                tipo_match = "Cliente Coincide (Diferencia Monto)"

        # -------------------------------------------------------------
        # REGISTRO DE RESULTADOS DE LA FILA
        # -------------------------------------------------------------
        if match_encontrado is not None:
            facturas_usadas.add(match_encontrado['Folio'])
            monto_factura = float(match_encontrado['Monto Total'])
            ...
            cruce_list.append({
                ...
                'Folios Factura(s)': match_encontrado['Folio'],
                'Entidad Matcheada': match_encontrado['Nombre Cliente'],
